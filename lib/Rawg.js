...
// RAWG's background_image is a full-size 1920px screenshot. Its media CDN
// serves pre-cropped copies under /media/crop/<w>/<h>/..., which is plenty
// for a cover thumbnail and a fraction of the download.
function coverUrl(backgroundImage) {
  if (!backgroundImage) return ""
  var url = String(backgroundImage)
  if (url.indexOf("/media/crop/") >= 0) return url
  // Covers live under /media/games/ or /media/screenshots/; the crop
  // prefix works for both.
...
function normalizeSearchHit(hit) {
  return {
    ...
    coverUrl: coverUrl(hit.background_image),
...
function parseDetails(rawJson) {
  ...
  return {
    ...
    coverUrl: coverUrl(data.background_image)
  }
}

function storesUrl(apiKey, id) {
  return BASE_URL + "/games/" + encodeURIComponent(id) + "/stores?key=" + encodeURIComponent(apiKey)
}
